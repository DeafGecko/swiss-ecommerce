...
<!-- Main Workstation Area -->
    <main class="flex-1 p-6 md:p-12 flex flex-col items-center justify-center">
      <div class="w-full max-w-6xl rounded-3xl p-8 md:p-12 grid grid-cols-1 md:grid-cols-2 gap-12 bg-[#EFEFF2] border border-[#C4C4C9] shadow-lg">
        
<!-- Left Column: Controls & Presets -->
        <section class="space-y-6 flex flex-col justify-center">
          
<!-- Preset Palettes & Swatch Library -->
          <div class="space-y-2">
            <div class="flex justify-between items-center">
              <span class="text-[10px] font-mono uppercase text-[#55555D] block">Color Palettes</span>
              <button @click="saveCurrentPalette()" class=" pr-2.5 text-[10px] font-mono uppercase text-[#111113] font-bold hover:underline">
            ...
            <div class="flex flex-wrap gap-2 max-h-32 overflow-y-auto p-1">
              <div v-for="(palette, index) in savedPalettes" :key="index"
                  class="group relative flex items-center rounded-lg border border-[#C4C4C9] bg-white overflow-hidden shadow-sm">
                <button @click="setPreset(palette.bg, palette.text)"
                        class="px-3 py-1.5 text-[10px] font-mono font-semibold uppercase tracking-wider flex items-center gap-2 hover:bg-[#111113] hover:text-white transition-colors">
...
<!-- Color Blindness Preview -->
          <div class="space-y-2">
            <span class="text-[10px] font-mono uppercase text-[#55555D] block">Color Blindness Preview</span>
...
<!-- Surface & On-Surface Pickers Matching Screenshot Layout -->
            ...
            <div class="flex items-center gap-3">
              <div class="relative">
                ...
                <input id="bg-color-picker" type="color" v-model="bgColor" 
                      class="w-14 h-14 shrink-0 cursor-pointer bg-transparent overflow-hidden shadow-inner [&::-webkit-color-swatch-wrapper]:p-0 [&::-webkit-color-swatch]:border-0 [&::-webkit-color-swatch]:rounded-2xl" />
              </div>
...
<!-- Shade & Tint Generator Strip -->
          <div class="space-y-2 pt-2 border-t border-[#C4C4C9]">
            <span class="text-[10px] font-mono uppercase text-[#55555D] block">Generated Scale (Shades & Tints)</span>
...
<!-- Contrast Ratio & WCAG Badges -->
          <div class="grid grid-cols-3 gap-4 pt-4 border-t border-[#C4C4C9]">
            <div class="col-span-2 bg-white p-5 rounded-2xl border border-[#C4C4C9]">
              <span class="block text-[10px] font-mono text-[#55555D] uppercase mb-1">Contrast Ratio</span>
        ...
        <section class="flex flex-col justify-between space-y-6">
          <div>
            <div class="flex justify-between items-center mb-2">
              <div class="flex items-center gap-3">
                <span class="text-[10px] font-mono uppercase text-[#55555D]">Live Preview</span>
                <!-- Typography / Font Style Selector -->
                <select v-model="activeFont" aria-label="Select typography style" class="text-[10px] font-mono bg-white border border-[#C4C4C9] rounded px-2 py-0.5 text-[#111113] focus:outline-none cursor-pointer">
...
<!-- Mode Badge with Clean Transparent Edit Button Matching Height -->
              <div class="flex items-center gap-2">
                <div class="flex items-center gap-1.5 text-[10px] font-mono uppercase text-[#55555D] bg-white px-3 py-1.5 rounded-lg border border-[#C4C4C9]">
                  <span>Mode: {{ currentSim }}</span>
                </div>
                <button @click="focusTitle" class="p-1.5 hover:text-black text-[#55555D] transition-colors cursor-pointer flex items-center justify-center" title="Click to edit preview text">
                  <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<!-- Applied Preview Box with Active Focus Glow -->
            <div class="p-8 md:p-10 rounded-2xl border-2 space-y-6 shadow-inner transition-all duration-150 min-h-95 flex flex-col justify-between focus-within:border-blue-500 focus-within:ring-4 focus-within:ring-blue-500/20"
                  :style="{ backgroundColor: bgColor, color: textColor, filter: activeFilterStyle }"
                  :class="activeFont">
                  
              <div class="space-y-6">
                <h2 ref="titleRef" contenteditable="true" @input="e => previewTitle = e.target.innerText" class="font-bold text-3xl tracking-tight leading-tight outline-none rounded px-1 -mx-1">Component Preview</h2>
                <p contenteditable="true" @input="e => previewText = e.target.innerText" class="text-base opacity-95 leading-relaxed outline-none rounded px-1 -mx-1">
                  Live testing for custom color pairings and legibility. Click here to edit and test your own words!
                </p>
              </div>

              <div class="pt-4">
                <button class="px-6 py-3 text-xs font-mono font-bold rounded-xl bg-current"
                        :style="{ color: bgColor, opacity: 0.95 }">
                  PRIMARY ACTION
                </button>
              </div>
            </div>
...
const previewText = ref('Live testing for custom color pairings and legibility.')
